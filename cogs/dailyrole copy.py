...
class DailyRoleAssigner(commands.Cog):
    ...
    async def assign_roles(self, interaction: discord.Interaction = None):
        ...
        for channel_id in LOGBOOK_CHANNELS:
            ...
            try:
                async for msg in channel.history(limit=None, oldest_first=False):
                    # stop scanning if 👍 reaction found
                    ...
                    if mention_tokens:
                        print(f"🔎 Found {len(mention_tokens)} mention tokens in message {msg.id}: {mention_tokens}")
                        for token in mention_tokens:
                            user_id = int(re.search(r"\d+", token).group())
                            all_mentions.append((msg, user_id))
                    else:
                        # fallback — use msg.mentions if regex found none
        ...
        # Processing mentions independently 
        for msg, user_id in all_mentions:
            ...
            print(f"   Current roles before assignment: {current_roles}")

            next_role = None
            # Sequential assignment logic — each mention advances one step
            ...
            if next_role:
                ...
                try:
                    ...
                    # remove Roll Call immediately if they reach Role 3
    ...
    # Slash command to manually trigger role log
